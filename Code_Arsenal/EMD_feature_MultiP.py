...
def get_wave_data(filename,pathname):
    ...
    if filename.split('.')[-1] == 'mp3':
        ...
        try:
            ...
            # 音频数据缺失不存在
            print(whatstime() + '读取文件 ' + file + ' 的时候出现以下错误：\n ',  e)           
            return wave_data, framerate, success
        ...
        try:        
            ...
            print(whatstime() + '读取文件 ' + new_file + ' 的时候出现以下错误：\n ',  e)            
            return  wave_data, framerate, success 
...
"""
将特征集3，4，特征集5，6解开成一个一层的字典
将所有特征集合成一个一层的字典
"""
def data_unwrapped(data):
    result = dict()
    for each_f in data:
        if each_f in ['f_cl1','f_cl2']:
            result.update(data[each_f])
        elif each_f in ['f_cl3s','f_cl4s','f_cl5s','f_cl6s']:
            data_group = data[each_f]            
            for each_k in data_group:
                result.update(data_group[each_k])  
    return result
...
q - 多线程运行时变量共享的队列
"""
def get_allfeatures(wav_info,ind,q):
    
    print(whatstime() + '共有%d'%ind[0] + '个音频，正在读取第%d'%ind[1] + '个')
    t_a = time()
    # 开启matlab引擎
...
#########   开始对信号进行特征集的提取
        ...
        print(whatstime() + '特征集1与特征集2特征提取完毕！')
        #########################################################
        ...
        print(whatstime() + '特征集3与特征集4特征提取完毕！')            
        #########################################################
        ...
        print(whatstime() + '特征集5与特征集6特征提取完毕！')
        all_feature = {'f_cl1':f_cl1,
                       'f_cl2':f_cl2,
                       'f_cl3s':f_cl3s,
                       'f_cl4s':f_cl4s,
                       'f_cl5s':f_cl5s,
                       'f_cl6s':f_cl6s}
        all_feature = data_unwrapped(all_feature)
        all_feature['machine_type'] = wav_info['机组状态'] 
        if ind_temp == 1:
            all_feature_df = pd.DataFrame(all_feature,index=[0])
        else:
            all_feature_df = all_feature_df.append(all_feature,ignore_index=True)
        ############################################################
...
#        else:
#            break
    q.put(all_feature_df)
    t_b = time()
    eng.quit()
     
    print(whatstime() + '第%d个音频读取完成,共耗时：%f'%(ind[1],(t_b - t_a)) + '秒')
...
if __name__ == '__main__':
    ...
    #   建立一个任务序列，每一百个音频文件的读取工作为一个task，每完成一个task产出一次结果
    task_num = len(wav_tag_table)//100
    task_group = [wav_tag_table[ ii*100 : (ii+1)*100 ] for ii in range(task_num + 1)]
    task_num = 0
    
    for each in task_group:
        task_num = task_num+1
        ################################
        ###         多进程处理         ###
        ################################    
    ...
    #####   4. 对每个录音进行综合特征提取
        for wav_info in each:
            
            ind = ind + 1
            file_prefix = 'normal'
    
            p.apply_async(get_allfeatures,args=(wav_info,[total_num,ind],q,))
            
         
        print(whatstime() + '正在等待所有子进程完成……')
        p.close()      
        p.join()
        print(whatstime() + '所有子进程已完成')
        ...
        all_feature_df = result[0]
        for each in result[1:]:
            all_feature_df = all_feature_df.append(each)
        
        # 将结果保存成excel格式，下次直接读入excel
        wbw = pd.ExcelWriter(file_prefix + 'data_source' + str(task_num) + '.xlsx')
        all_feature_df.to_excel(wbw,'data_source')
        wbw.save();
        print(whatstime() + '第 %d 个文件保存完毕！'%task_num)

    print(whatstime() + '程序总耗时：%f 秒'%(time() - t_start))